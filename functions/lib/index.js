...
exports.passUser = exports.likeUser = exports.getTopCandidates = exports.submitStudentId = exports.confirmUniversityVerification = exports.requestUniversityVerification = exports.postMessage = exports.createMatch = exports.listRooms = exports.createUser = exports.health = void 0;
...
exports.listRooms = functions.https.onRequest(async (_req, res) => {
    try {
...
exports.postMessage = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        await db.collection('messages').doc(conversationId).set({ messages: admin.firestore.FieldValue.arrayUnion(message) }, { merge: true });
        res.status(201).json({ ok: true });
    }
    catch (err) {
        res.status(500).json({ error: err.message });
...
exports.requestUniversityVerification = functions.https.onRequest(async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).send('Method Not Allowed');
        return;
    }
    try {
        const { userId, email, university } = req.body;
        if (!userId || !email) {
...
exports.confirmUniversityVerification = functions.https.onRequest(async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).send('Method Not Allowed');
        return;
    }
...
exports.submitStudentId = functions.https.onRequest(async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).send('Method Not Allowed');
        return;
    }
...
exports.getTopCandidates = functions.https.onRequest(async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).send('Method Not Allowed');
        return;
    }
...
async function hasReciprocalLike(fromUserId, toUserId) {
...
exports.passUser = functions.https.onRequest(async (req, res) => {
    if (req.method !== 'POST') {
        res.status(405).send('Method Not Allowed');
        return;
    }
